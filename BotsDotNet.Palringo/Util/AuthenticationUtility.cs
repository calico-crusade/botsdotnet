...
namespace BotsDotNet.Palringo.Util
{
    ...
    {
        byte[] GenerateAuth(byte[] keyIv, byte[] password);
    }
    ...
    public class AuthenticationUtility : IAuthenticationUtility
    {
        ...
        private class Salsa20 : SymmetricAlgorithm
        {
            /// <summary>
        ...
        public byte[] GenerateAuth(byte[] keyIv, byte[] password)
        {
            byte[] iv = new byte[8];
            int i = 0;
            ...
            for (i = 0; i < 16; i++)
            {
                dataToEncrypt[i] = keyIv[i];
                dataToEncrypt[i + 16] = rnd[i];
            }


            byte[] result = DoSalsa20(iv, authKey, dataToEncrypt);
            return result;
        }

        private byte[] DoubleMD5(byte[] abyte0, byte[] abyte1)
        {
